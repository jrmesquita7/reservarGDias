...
$equipamentos = $pdo->query("SELECT * FROM itens WHERE status = 'Disponível'ORDER BY nome")->fetchAll(PDO::FETCH_ASSOC);
$horarios = $pdo->query("SELECT * FROM horarios ORDER BY id")->fetchAll(PDO::FETCH_ASSOC);

$stmt = $pdo->prepare("
    SELECT r.*, u.nome AS nome_usuario, h.descricao AS horario_desc, i.nome AS nome_item 
    FROM reservas r
...
foreach ($reservas as $res) {
    $reservas_map[$res['id_horario']][$res['id_item']] = $res;
}
?>
...
<head>
    ...
    <title>Painel de Reservas de Equipamentos - <?= date('d/m/Y', strtotime($data_atual)) ?></title>
        ...
        <?php foreach ($equipamentos as $item): ?>
            <div class="card">
                ...
                <?php foreach ($horarios as $horario): ?>
                    <?php 
                        $res = $reservas_map[$horario['id']][$item['id']] ?? null; 
